...
!  PUBLIC :: ReadInput, DeallocateInputArrays
  ...
  REAL(8), ALLOCATABLE :: alpha(:), couplingA(:,:), couplingExponent(:,:)
  REAL(8) :: beta
  ...
  CONTAINS
    ...
    SUBROUTINE AllocateInputArrays
      ...
      ALLOCATE(couplingA(nStates,nStates));                               couplingA = 0.0d0
...
!*
    ...
    SUBROUTINE DeallocateInputArrays
      ...
      IF (ALLOCATED(couplingA))        DEALLOCATE(couplingA)
...
!*
    ...
    SUBROUTINE ReadEVBParameters(logUnit)
      ...
      IF (fileExists) THEN
        ...
        DO i = 1, nStates
          DO j = i+1, nstates
            READ(prmUnit,*) dummy, dummy, couplingA(i,j), couplingExponent(i,j)
          ENDDO
        ENDDO
...
!*
    ...
    SUBROUTINE PrintEVBParameters(logUnit)
      ...
      DO i = 1, nStates
        DO j = i+1, nStates
          WRITE(logUnit,'(2I4,2F7.3)') i, j, couplingA(i,j), couplingExponent(i,j)
        ENDDO
      ENDDO
      WRITE(logUnit,*)
